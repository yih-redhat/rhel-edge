...
import_ov () {
    ...
    fdo-owner-tool import-ownership-vouchers "$(tr "[:upper:]" "[:lower:]" <<< "${DATABASE_DRIVER}")" "${POSTGRES_DB_URL}" "${MANUFACTURER_EXPORT_DIR}"
    rm -rf "${MANUFACTURER_EXPORT_DIR}"
}
...
if [[ $DB_RANDOM_CHOICE == 0 ]]; then
    ...
    SQLITE_DB_URL="sqlite://${DATABASE_DIR}/fido-device-onboard.db"
    true > ${DATABASE_FILE}
...
#         /usr/lib/systemd/system/fdo-rendezvous-server.service
    ...
    sudo /usr/local/bin/yq -yi 'del(.ownership_voucher_store_driver.Directory)' /etc/fdo/manufacturing-server.conf.d/manufacturing-server.yml
    sudo /usr/local/bin/yq -yi '.ownership_voucher_store_driver += {"Sqlite":{"server": "Manufacturer", "url": "'$SQLITE_DB_URL'"}}' /etc/fdo/manufacturing-server.conf.d/manufacturing-server.yml
    sudo /usr/local/bin/yq -yi 'del(.ownership_voucher_store_driver.Directory)' /etc/fdo/owner-onboarding-server.conf.d/owner-onboarding-server.yml
    sudo /usr/local/bin/yq -yi '.ownership_voucher_store_driver += {"Sqlite":{"server": "Owner", "url": "'$SQLITE_DB_URL'"}}' /etc/fdo/owner-onboarding-server.conf.d/owner-onboarding-server.yml
    sudo /usr/local/bin/yq -yi 'del(.storage_driver.Directory)' /etc/fdo/rendezvous-server.conf.d/rendezvous-server.yml
    sudo /usr/local/bin/yq -yi '.storage_driver += {"Sqlite":{"server": "Rendezvous", "url": "'$SQLITE_DB_URL'"}}' /etc/fdo/rendezvous-server.conf.d/rendezvous-server.yml
else
    ...
    POSTGRES_DB_URL="postgresql://${POSTGRES_USERNAME}:${POSTGRES_PASSWORD}@${POSTGRES_IP}/${POSTGRES_DB}"
    ...
    sudo /usr/local/bin/yq -yi 'del(.ownership_voucher_store_driver.Directory)' /etc/fdo/manufacturing-server.conf.d/manufacturing-server.yml
    sudo /usr/local/bin/yq -yi '.ownership_voucher_store_driver += {"Postgres":{"server": "Manufacturer", "url": "'$POSTGRES_DB_URL'"}}' /etc/fdo/manufacturing-server.conf.d/manufacturing-server.yml
    sudo /usr/local/bin/yq -yi 'del(.ownership_voucher_store_driver.Directory)' /etc/fdo/owner-onboarding-server.conf.d/owner-onboarding-server.yml
    sudo /usr/local/bin/yq -yi '.ownership_voucher_store_driver += {"Postgres":{"server": "Owner", "url": "'$POSTGRES_DB_URL'"}}' /etc/fdo/owner-onboarding-server.conf.d/owner-onboarding-server.yml
    sudo /usr/local/bin/yq -yi 'del(.storage_driver.Directory)' /etc/fdo/rendezvous-server.conf.d/rendezvous-server.yml
    sudo /usr/local/bin/yq -yi '.storage_driver += {"Postgres":{"server": "Rendezvous", "url": "'$POSTGRES_DB_URL'"}}' /etc/fdo/rendezvous-server.conf.d/rendezvous-server.yml
fi
